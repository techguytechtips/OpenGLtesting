void Camera(vec3 *position, vec3 orientation, GLuint u_cam, float FOVdeg, unsigned int winWidth, unsigned int winHeight, float viewMin, float viewMax, vec3 up){ 
		// create the 4x4 identity matrices
		mat4 view;
		mat4 proj;
		glm_mat4_identity(view);
		glm_mat4_identity(proj);
		// create a mat4 for the sum of
		// proj multiplied by view
		mat4 prmulv;
		...
		glm_mat4_mul(proj, view, prmulv);

		glUniformMatrix4fv(u_cam, 1, GL_FALSE, *prmulv);
}
